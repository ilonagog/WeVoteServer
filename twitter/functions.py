...
def retrieve_twitter_user_info(twitter_user_id=0, twitter_handle='', twitter_api_counter_manager=None, parent=None):
    ...
    if LOG_OAUTH:
        logger.error('(Ok) tweepy client init. in retrieve_twitter_user_info -- twitter_handle: %s', twitter_handle)
    ...
    try:
        if positive_value_exists(twitter_handle):
            ...
            if LOG_OAUTH:
                logger.error('(Ok) tweepy client get_user #1 in retrieve_twitter_user_info -- twitter_handle: %s',
                             twitter_handle)
            ...
            if LOG_OAUTH:
                logger.error('(Ok) tweepy client get_user #2 in retrieve_twitter_user_info -- twitter_handle: %s',
                             twitter_handle)
            try:
                counter = create_detailed_counter_entry(
                    kind_of_action='get_user',
...
def retrieve_twitter_user_info_from_handles_list(
    ...
    if retrieve_from_twitter:
        try:
            if LOG_OAUTH:
                logger.error('%s', '(Ok) tweepy client init in retrieve_twitter_user_info_from_handles_list')
            client = tweepy.Client(
                bearer_token=TWITTER_BEARER_TOKEN,
                consumer_key=TWITTER_CONSUMER_KEY,
                consumer_secret=TWITTER_CONSUMER_SECRET,
            ...
            if LOG_OAUTH:
                logger.error('%s', '(Ok) tweepy client get_users in retrieve_twitter_user_info_from_handles_list')
