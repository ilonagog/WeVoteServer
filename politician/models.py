...
class Politician(models.Model):
    ...
    # Politician has claimed this profile - This is the master value, and is_claimed_profile in candidate table is copy
    is_claimed_profile = models.BooleanField(default=False, null=False)
    is_claimed_profile_analysis_complete = models.BooleanField(default=False)
    is_claimed_profile_date_time = models.DateTimeField(null=True)
    # Every politician has one default CampaignX entry that follows them over time. Campaigns with
    #  a linked_politician_we_vote_id are auto-generated by We Vote. TODO: Clarify this is the master data
    ...
    organization_analysis_needed = models.BooleanField(default=True)
    organization_manual_intervention_needed = models.BooleanField(default=False)
    politician_has_two_linked_organizations = models.BooleanField(default=False)
...
class PoliticianSupporter(models.Model):
    """
    We use the PoliticianSupporter for our "Manage My Candidates: Import & Invite Voters" feature.
    These are people invited by the Politician to support them. They may or may not have an existing WeVote account
    when invited.
    """
    ...
    email_we_vote_id = models.CharField(max_length=255, null=True, blank=True, unique=True, db_index=True)
    invited_by_voter_we_vote_id = models.CharField(max_length=255, null=True, db_index=True)
        ...
        try:
            ...
            for full_name in full_name_list:
                if positive_value_exists(full_name):
                    ...
                    # queryset = filter_results['queryset']
...
#             politician_entry.delete()
    ...
    def retrieve_politician_list(
            ...
            politicians_limit=100,
            read_only=False,
            search_string='',
    ):
        """

        :param campaignx_we_vote_id_list:
        :param index_start:
        :param limit_to_this_state_code:
        :param politician_we_vote_id_list:
        :param politicians_limit:
        :param read_only:
        :param search_string:
        :return:
        """
        ...
        try:
            ...
            if len(campaignx_we_vote_id_list):
            ...
            if positive_value_exists(search_string):
                ...
                for search_word in search_words:
                    ...
                    # We want to find politicians with *any* of these values
                    new_filter = Q(bluesky_handle__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(first_name__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(full_name_assembled__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(google_civic_candidate_name__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(google_civic_candidate_name2__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(google_civic_candidate_name3__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(instagram_handle__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(last_name__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(linkedin_url__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(middle_name__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(political_party__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_email__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_email2__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_email3__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_name__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_twitter_handle__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_twitter_handle2__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_twitter_handle3__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_twitter_handle4__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_twitter_handle5__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_url__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_url2__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_url3__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_url4__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(politician_url5__icontains=search_word)
                    filters.append(new_filter)
                    new_filter = Q(we_vote_id__icontains=search_word)
                    filters.append(new_filter)
            ...
            queryset = queryset.order_by('-politician_ultimate_election_date', '-twitter_followers_count', '-supporters_count')
            if politicians_limit > 0:
                if index_start > 0:
                    politician_list = queryset[index_start:politicians_limit]
                else:
                    politician_list = queryset[:politicians_limit]
            else:
                politician_list = list(queryset)
        ...
        returned_count = len(politician_list)
        ...
        if keep_looking_for_duplicates and len(twitter_handle_list) > 0:
            try:
                ...
                if positive_value_exists(ignore_politician_id_list):
                    queryset = queryset.exclude(we_vote_id__in=ignore_politician_id_list)
    ...
    def fetch_politicians_from_non_unique_identifiers_count(
        ...
        if keep_looking_for_duplicates and len(twitter_handle_list) > 0:
            try:
                ...
                politician_count = queryset.count()
    ...
    def retrieve_politicians_with_no_gender_id(self, start=0, count=15, show_unknowns=True):
        ...
        queryset = queryset.filter(gender=UNKNOWN)
        number_of_rows = queryset.count()
        queryset = queryset.order_by('politician_name')
        queryset = queryset[start:(start + count)]
        politician_list_objects = list(queryset)
        results_list = []
    ...
    def retrieve_politicians_with_misformatted_names(start=0, count=15, read_only=False):
        ...
        queryset = queryset.filter(politician_name__regex=r'.*?[A-Z][A-Z][A-Z].*?(?<!III)').\
